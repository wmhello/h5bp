...
(function ($) {
    ...
    var Spinning = function (el, options) {
        ...
        this.min = parseFloat(el.attr('min')) || parseFloat(this.options.min) || 0;
        this.max = parseFloat(el.attr('max')) || parseFloat(this.options.max) || 0;
    ...
    Spinning.rules = {
        ...
        currency: {min: 0.00, max: null, step: 0.10, precision: 2},
        excurrency: {min: 0.10, max: null, step: 0.10, precision: 2},
        weight: {min: 0.01, max: 30, step: 0.10, precision: 2},
        quantity: {min: 1, max: 999, step: 1, precision: 0},
        quantity1: {min: 0, max: 999, step: 1, precision: 0},
    ...
    Spinning.prototype = {
        spin: function (dir) {
            if (this.$el.parents('.disabled, :disabled').length == 0) {
        ...
        value: function (v) {
            ...
            if (valid !== 0) {
                if (valid === -1) {
                    this.$el.trigger('validate.min.spinner', [v, this.min]);
                    v = this.min;
                } else {
                    this.$el.trigger('validate.max.spinner', [v, this.max]);
                    v = this.max
                }
            }
